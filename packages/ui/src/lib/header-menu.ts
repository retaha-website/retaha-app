...
export interface MenuOpts {
  /** Prefix für Backoffice-Routen. '' im Backoffice (relativ), BACKOFFICE_URL in der Gast-App (absolut). */
  base?: string;
  /** Prefix für den Gast-App-Loyalty-Link. GUEST_APP_URL im Backoffice, '' in der Gast-App (relativ). */
  guestBase?: string;
}
...
// Geteilte App-Menü-Sektionen (Backoffice + Gast-App). Labels via bt(); URLs über
// base/guestBase parametrisiert, damit dieselbe Quelle aus beiden Apps korrekt linkt.
export function getMenuSections(lang: LanguageCode, opts: MenuOpts = {}): MenuSection[] {
  const base = opts.base ?? '';
  const guestBase = opts.guestBase ?? '';
  const bo = (route: string) => `${base}${route}`;
  return [
    {
