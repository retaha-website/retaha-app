...
import { buildSystemPrompt, type EveContext } from '@retaha/eve';
import { EVE_TOOLS, isActionTool } from '@retaha/eve';
import { executeTool, type EveExecutionContext } from '@retaha/eve';
import { getTranslatedKnowledge } from '@retaha/eve';
import type { Lang } from '@retaha/i18n';
import { normalizeLang } from '@retaha/i18n';
...
async function loadEveChatContext(sb: any, hotelId: string, stayOrShowcaseId: string, requestedLang?: Lang, isShowcase = false): Promise<EveContext | null> {
  ...
  if (isShowcase) {
    const [hotelRes, settingsRes, knowledgeRes, showcaseRes] = await Promise.all([
      ...
      sb.from('hotel_settings').select('eve_enabled, eve_name, eve_tonality, eve_custom_persona, eve_tuning_rules, guest_address_form, wifi_ssid, wifi_password, wifi_speed_mbits, breakfast_start_time, breakfast_end_time, breakfast_location_de, breakfast_location_en, breakfast_location_fr, breakfast_location_es').eq('hotel_id', hotelId).maybeSingle(),
