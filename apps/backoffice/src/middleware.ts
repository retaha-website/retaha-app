 ...
 *      /profile/security?required=true. (b) Login-Challenge — Faktor aktiv + kein gültiger
...
// durchgehen). Force-Setup nimmt zusätzlich /profile aus (= sein Redirect-Ziel);
// die Login-Challenge gilt AUCH auf /profile (sie geht aufs auth-Subdomain → kein Loop).
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // Präzedenz: kein Faktor + Hotel-Pflicht → Setup (/profile/security?required=true);
  ...
  if (roles.length > 0 && !isApiOrAuthPath(url.pathname)) {
    ...
    if (hotelId) {
      ...
      // Force-Setup — /profile ausnehmen (= Redirect-Ziel, sonst Loop).
      if (!url.pathname.startsWith('/profile') && shouldForceSetup(mfaStatus, policy)) {
        return Response.redirect(new URL('/profile/security?required=true', url.origin).toString(), 302);
      }
